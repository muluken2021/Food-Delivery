...
const OrderManager = () => {
  ...
  const filteredOrders = orders.filter((o) => {
    if (activeTab === "active") return ["Pending", "Accepted", "Food Processing"].includes(o.status);
    return ["Delivered", "Cancelled", "Canceled"].includes(o.status);
  });

  const removeOrder = async (orderId) => {
    if (!window.confirm(t('order_confirm_remove'))) return;
    try {
      const token = localStorage.getItem("token");
  ...
  return (
    <div className="w-full max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-gray-100 pb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">{t('order_your_orders')}</h2>
          <p className="text-sm text-gray-500">{t('order_manage_desc')}</p>
        </div>
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl">
          ...
          >
            <Package size={16} /> {t('order_tab_active')}
          </button>
          <button
          ...
          >
            <History size={16} /> {t('order_tab_history')}
          </button>
        </div>
      </div>
      ...
      <div className="min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-500 font-medium">{t('order_none_found')}</p>
          </div>
        ) : (
